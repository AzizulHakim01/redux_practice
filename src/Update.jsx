...
import { updateProduct } from './UserReducer'

const Update = () => {
  const dispatch = useDispatch()
  const product = useSelector(state => state.product)
  const {id} = useParams()
  const existingProduct = product.filter( e =>e.id == id)
  const {productName, brand, category, description, price} = existingProduct[0];


  const [uproductName, setProductName] = useState(productName)
